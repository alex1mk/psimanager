  ...
  const handleGenerate = async (
    ...
    if (isRangeEmpty) {
      setAlert({
        type: "info",
        message:
          "Não é possível gerar relatórios no momento por falta de dados no período selecionado.",
      });
      return;
    }
    ...
    setAlert({
      type: "info",
      message: `Gerando relatório ${type} de ${startDisplay} até ${endDisplay}...`,
    });
  ...
  const handleRetroUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setTimeout(() => {
        ...
        setAlert({
          type: "success",
          message: `Arquivo "${file.name}" processado e incluído no histórico financeiro.`,
        });
        e.target.value = "";
      }, 2000);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-verde-botanico">
          Gere documentos fiscais e administrativos automaticamente.
        </p>
      </div>
        ...
        {reportTypes.map((report) => (
          ...
          >
            <div>
              ...
              <h3 className="text-lg font-bold text-verde-botanico mb-2">
                {report.title}
              </h3>
            ...
            <div className="space-y-4 pt-4 border-t border-verde-botanico/10">
                ...
                <div className="flex items-center bg-bege-calmo/50 border border-verde-botanico/10 rounded-xl shadow-inner relative">
                  <div className="flex-1">
                    <DatePickerInput
                      value={
                        new Date(
                          (reportRanges as any)[report.id].start + "T12:00:00",
                        )
                      }
                      onChange={(date) =>
                        handleRangeChange(
                      ...
                      maxDate={
                        new Date(
                          (reportRanges as any)[report.id].end + "T12:00:00",
                        )
                      }
              ...
              <button
                onClick={() =>
                  handleGenerate(report.id, (reportRanges as any)[report.id])
                }
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <h3 className="font-bold text-verde-botanico text-lg flex items-center gap-2 font-sans">
              ...
              <span className="text-[10px] bg-bege-calmo text-verde-botanico px-2 py-0.5 rounded-full border border-verde-botanico/20 uppercase font-bold tracking-wider">
                Novo
              </span>
            </h3>
            <p className="text-verde-botanico text-sm mt-1 max-w-xl">
              Precisa computar despesas antigas para relatórios passados? Faça o
              upload de recibos digitalizados ou planilhas aqui. O sistema irá
              processar e incluir na data detectada no arquivo.
            </p>
          </div>
        </div>
        <div className="w-full md:w-auto">
          <label
            className={`flex items-center justify-center gap-2 px-6 py-3 bg-white border border-verde-botanico/20 text-verde-botanico rounded-xl hover:bg-bege-calmo font-bold text-sm cursor-pointer transition-all shadow-sm ${isUploadingRetro ? "opacity-70 cursor-wait" : ""}`}
          >
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-bold text-verde-botanico text-lg font-sans">
            Histórico de Relatórios Gerados
          </h3>
          <button className="text-sm text-verde-botanico hover:underline font-bold">
            Ver todos
          </button>
        </div>
          ...
          <table className="w-full text-sm text-left text-verde-botanico">
            ...
            <tbody>
              ...
              <tr>
                <td
                  colSpan={4}
                  className="px-4 py-8 text-center text-verde-botanico italic"
                >
