...
const Agenda: React.FC = () => {
    ...
    return days.map((day) => {
      ...
      const dayAppointments = appointments
        ...
        .filter((app) => {
          ...
          if (
            filterPatient &&
            !app.patientName.toLowerCase().includes(filterPatient.toLowerCase())
          )
            return false;
          return true;
        })
  ...
  const handleConnectGoogle = async () => {
    ...
    try {
      ...
      setNotificationStatus({
        type: "success",
        message: "Conectado e sincronizado com Google Calendar!",
      });
    } catch (error: any) {
  ...
  const handleConfirm = useCallback(
    async (id: string, name: string) => {
      setNotificationStatus({
        type: "info",
        message: `Enviando confirmações para ${name}...`,
      });

      try {
        // 1. Send sequential notifications to avoid server/spam blocks
        const notificationTypes = ["whatsapp", "email"] as const;
        for (const type of notificationTypes) {
          console.log(`[Agenda] Disparando ${type} para ${name}`);
          await sendNotification(type, name, "Hoje");
        }
        ...
        setNotificationStatus({
          type: "success",
          message: `Presença de ${name} confirmada! Notificações enviadas com sucesso.`,
        });
        ...
        console.error("[Agenda] Falha ao processar confirmação:", error);
        setNotificationStatus({
          type: "error",
          message: `Erro ao enviar notificações para ${name}.`,
        });
      }
      ...
      setSelectedEvent((prev: any) => {
        if (prev && prev.resource.id === id) {
          return {
            ...prev,
            resource: { ...prev.resource, status: "confirmed" },
          };
        }
        return prev;
      });
  ...
  const onEventDrop = (data: {
    ...
    if (event.resource.source === "google") {
      setNotificationStatus({
        type: "info",
        message:
          "Eventos do Google Calendar devem ser editados na plataforma original.",
      });
      return;
    }
  ...
  const confirmReschedule = async () => {
    ...
    try {
      ...
      setAppointments((prev) =>
        prev.map((app) => (app.id === event.id ? updatedAppointment : app)),
      );
  ...
  const handleCreateAppointment = async (e?: React.FormEvent) => {
    ...
    if (
      ...
      setNotificationStatus({
        type: "error",
        message: "Preencha paciente, data e horário.",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6 h-full flex flex-col p-6 bg-bege-calmo rounded-3xl border border-verde-botanico/10 shadow-inner relative overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10 font-sans">
        <div>
          <h2 className="text-3xl font-sans font-bold text-verde-botanico">
            Agenda
          </h2>
      ...
      <div className="bg-white/60 backdrop-blur-sm p-4 rounded-2xl border border-verde-botanico/10 flex flex-col sm:flex-row gap-4 items-center shadow-sm relative z-10">
        ...
        <div className="flex items-center gap-3 border-l border-r border-verde-botanico/10 px-6 mx-2 relative">
          <span className="text-xs text-verde-botanico/50 font-bold uppercase tracking-widest">
            DATA
          </span>
                ...
                <div className="day-picker-popover">
                  <DayPicker
                    ...
                    onMonthChange={(newMonth) => {
                      console.log(
                        "[Agenda] Navegação para:",
                        newMonth.toISOString(),
                      );
        ...
        <div className="calendar-content">
          ...
          <div className="calendar-grid">
            {daysInMonth.map((day) => (
              ...
              >
                <div className="calendar-day-number font-sans">
                  {day.dayNumber}
                </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-xl font-bold text-verde-botanico">
                    {selectedEvent.title}
                  </h4>
              ...
              <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                <div className="flex justify-between">
                  <div>
                    <div className="text-xs font-semibold text-verde-botanico uppercase mb-1">
                      Status
                    </div>
              ...
              {selectedEvent.resource.source === "google" && (
                <p className="text-xs text-center text-verde-botanico italic mt-4">
                  Este evento foi importado do Google Calendar e não pode ser
                  editado aqui.
                </p>
              )}
            </div>
            ...
            <div className="p-6">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-verde-botanico text-sm mt-1">
                    Você está movendo a consulta de{" "}
                    <strong className="text-verde-botanico">
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 flex items-center justify-between gap-2 mb-6">
                <div className="text-center flex-1">
                  <p className="text-xs text-verde-botanico uppercase font-semibold">
                    De
                  </p>
              ...
              <p className="text-xs text-verde-botanico mb-6 bg-yellow-50 p-2 rounded border border-yellow-100">
                ⚠️ Confirme o novo horário acima. Uma notificação será enviada
                automaticamente.
              </p>
