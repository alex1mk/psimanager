...
function sanitizePatientData(excelRow: any) {
  const rawPaymentType = excelRow.payment_type?.trim();
  const mappedPaymentType = PAYMENT_TYPE_MAP[rawPaymentType];
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        const payment = findVal(['pagamento', 'tipo', 'payment', 'payment_type']);
        const fixedDay = findVal(['dia', 'dia fixo', 'fixed_day']);
        const fixedTime = findVal(['horario', 'hora', 'time', 'fixed_time']);

        if (name && email) {
          excelRows.push({
            name, email, phone, payment_type: payment, fixed_day: fixedDay, fixed_time: fixedTime
          });
        }
      });
      ...
      for (const row of excelRows) {
        try {
          const sanitized = sanitizePatientData(row);
          ...
          if (error) {
            ...
            results.failed.push({
              name: sanitized.name,
              reason: isDuplicate ? 'Email já cadastrado' : error.message
            });
          } else {
            results.success.push(sanitized.name);
          }
        } catch (err: any) {
          results.failed.push({
            name: row.name || 'Linha sem nome',
            reason: err.message
          });
        }
      }
      ...
      const message = results.failed.length > 0
        ? `Importação parcial: ${results.success.length} importados. Erros: ${results.failed.slice(0, 3).map(f => f.name).join(', ')}`
        : `${results.success.length} pacientes importados com sucesso!`;
      ...
      console.error(error);
      setAlert({ type: 'error', message: 'Erro crítico ao processar planilha.' });
    } finally {
